...
import { COURSE_LEVELS, courseChapterHref } from "@/app/course/course-data";
...
export default function CoursePage() {
  ...
  const publishedLessons = ALL_GRAMMAR_LESSONS.filter((lesson) => COURSE_LEVELS.some((level) => lesson.id.startsWith(`${level.toLowerCase()}-`)));
  ...
  const nextLevel = nextLesson.id.slice(0, 2).toUpperCase() as (typeof COURSE_LEVELS)[number];
    ...
    <main className="site-shell course-home" id="top">
      ...
      <section className="a1-roadmap" id="roadmap" aria-labelledby="course-roadmap-title">
        ...
        {COURSE_LEVELS.map((level) => {
          const lessons = ALL_GRAMMAR_LESSONS.filter((lesson) => lesson.id.startsWith(`${level.toLowerCase()}-`));
          ...
          return <section className="course-level-roadmap" id={`roadmap-${level.toLowerCase()}`} key={level}>
            ...
            <div className="a1-module-list">
              {GRAMMAR_MODULES.filter((module) => module.level === level).map((module) => <section key={module.id} className="a1-module"><header><span>Module {module.number}</span><h3>{module.title}</h3><p>{module.description}</p></header><div>{module.lessons.map((lesson) => {
