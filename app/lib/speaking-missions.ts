...
export function getSpeakingMission(taskId: string): SpeakingMission | null {
  const task = taskId.startsWith("active-") ? activeTask(taskId) : getChapterOutputTask(taskId);
  if (!task) return null;
  if (taskId.startsWith("active-")) return { id: taskId, title: task.title, goal: task.writing, role: "a patient everyday conversation partner", opening: task.questions[0], patternId: "other", level: task.level, turns: task.questions.length, chapter: task.chapter, questions: task.questions, speakingSize: task.speakingSize, rubric: task.rubric, grammarFocus: "Communicate clearly using level-appropriate everyday language" };
