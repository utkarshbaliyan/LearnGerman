...
export function filterReadingStories(stories: ReadingSummary[], level: ReadingLevel, topic: string, query: string) {
  const terms = normalise(query.trim()).split(/\s+/).filter(Boolean);
  return stories.filter(story => story.level === level && (topic === 'all' || story.topics.includes(topic)) && terms.every(term => normalise(`${story.title} ${story.goal} ${story.topics.map(key => `${key} ${(topics as Record<string, string>)[key] ?? ''} ${aliases[key] ?? ''}`).join(' ')} ${story.grammar}`).includes(term)));
}
