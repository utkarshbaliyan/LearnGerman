...
export function readingGlosses(story: ReadingStory) {
  ...
  for (const token of story.text.match(/[\p{L}]+(?:[-’'][\p{L}]+)*/gu) ?? []) {
    ...
    const meaning = story.wordGlosses?.[key] || (story.courseChapter === null ? (expandedGlosses as Record<string, string>)[key] : '') || (additions as Record<string, string>)[key] || meaningFor(token);
