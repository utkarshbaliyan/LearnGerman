...
export function SpeakingWorkspace({ taskId }: { taskId: string }) {
  ...
  const latest = record?.session.attempts.filter((item) => item.status === "complete").at(-1);
  return <div className="speaking-workspace">
    {!signedIn ? <p><Link href="/account">Sign in</Link> to start a speaking mission and save your progress.</p> : !record ? <p>Loading your speaking practice…</p> : <>
      <h3>{record.mission.title}</h3><p>{record.mission.goal}</p>
      {!state || state.ended && !repairing ? <div><p>{state?.ended ? "Mission finished. Review your feedback below." : "Choose how you want to practise."}</p><div className="writing-repair-actions"><Button variant={mode === "conversation" ? "default" : "outline"} onClick={() => setMode("conversation")} aria-pressed={mode === "conversation"}>Conversation: feedback at the end</Button><Button variant={mode === "focus" ? "default" : "outline"} onClick={() => setMode("focus")} aria-pressed={mode === "focus"}>Focused: feedback after each turn</Button><Button disabled={busy} onClick={() => void act("start")}>{state ? "Start another mission" : "Start mission"}</Button></div></div> : <>
        ...
        <label className="writing-photo-consent"><input type="checkbox" checked={consent} disabled={busy || recording} onChange={(event) => setConsent(event.target.checked)} /><span>I agree to record my voice and send it for transcription when I choose Read recording.</span></label>
      ...
      {repairing && <p role="status">Record the previous response again using the feedback below. This is assisted repair, not a new conversation turn.</p>}
      ...
      <p className="ai-tutor-privacy">Recordings stay in this page and are sent only for transcription. LeseLaut saves confirmed text, turns and feedback until you delete them. Feedback assesses language in the transcript, not pronunciation. All tutor actions share 20 AI requests per day; a focused response or final conversation response uses two.</p>
      <div className="writing-repair-actions"><Button variant="outline" disabled={busy || recording} onClick={() => void load().catch((e) => setError(e.message))}>Reload saved work</Button>{deleteOpen ? <><span>Delete all speaking history for this chapter?</span><Button variant="destructive" disabled={busy || recording} onClick={() => void act("delete")}>Delete history</Button><Button variant="outline" onClick={() => setDeleteOpen(false)}>Cancel</Button></> : <Button variant="ghost" disabled={busy || recording} onClick={() => setDeleteOpen(true)}>Delete speaking history</Button>}</div>
    </>}
