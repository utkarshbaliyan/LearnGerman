...
function ReceptionTask({activity,audio,record,enabled,owner,onStart,onSupport,onCheck}:{
 ...
 return <section className="reception-task" aria-label={activity.title}>
  ...
  {listening?<div className="reception-player">
   ...
   <div className="reception-audio-options"><label>Speed <select defaultValue="1" onChange={event=>{const rate=Number(event.target.value);if(rate<1)onSupport();if(player.current)player.current.playbackRate=rate;}}><option value="0.85">Slower</option><option value="1">Normal</option></select></label><small>Synthetic German voices · {audio?timeLabel(audio.duration):'audio unavailable'}</small></div>
