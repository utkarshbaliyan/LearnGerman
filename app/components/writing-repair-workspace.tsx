  ...
  return <div className="writing-workspace writing-repair">
    ...
    {!signedIn ? <p><Link href="/account">Sign in</Link> to save drafts and practise with feedback.</p> : <>
      {!record ? <p>Load your saved writing to begin.</p> : <>
        ...
        {!taskId.startsWith("active-") && task?.starter && <p className="writing-starter"><span>Start with:</span> <b lang="de">{task.starter}</b></p>}
        ...
        <div className="writing-repair-actions"><Button variant="outline" disabled={busy || autoSaving || conflict || recovery !== null || draft === record.session.draft} onClick={() => void act("draft")}>Save draft</Button><Button disabled={busy || autoSaving || conflict || recovery !== null || draft.trim().length < (taskId.startsWith("active-") ? 2 : 10) || (draft === latest?.answer && !(taskId.startsWith("active-") && (!latest?.feedback?.development || latest.feedback.development.explanation.startsWith("The tutor could not reliably"))))} onClick={() => void act("check")}>{busy ? "Saving / checking…" : latest ? "Check my revision" : taskId.startsWith("active-") ? "Check my writing" : "Get a hint"}</Button></div>
        <p className="writing-guidance" role="status">{autoSaving ? "Saving…" : draft !== record.session.draft ? "Unsaved changes" : "Saved"}</p>
        {latest && taskId.startsWith("active-") && !latest.feedback?.development && <p>Recheck this saved answer to assess whether it includes enough detail.</p>}
