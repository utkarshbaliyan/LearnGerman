...
export function TutorRepairFeedback({ attempt, busy, onReveal, onAction }: { attempt: WritingAttempt; busy: boolean; onReveal: () => void; onAction: (action: FeedbackAction) => void }) {
  const issues = [...(attempt.feedback?.issues ?? [])].sort((a, b) => a.start - b.start);
  return <section className="writing-repair-feedback" aria-label="Language feedback"><h3>{attempt.feedback?.taskSuccess && !attempt.disputes?.length ? "Task completed in practice" : "Your repair focus"}</h3><p>{attempt.feedback?.summary}</p><HighlightedAnswer attempt={attempt} />
    {issues.map((issue, index) => <article key={issue.start}><b>{index + 1}. {issue.category} · {issue.kind === "style" ? "Optional style suggestion" : "Error to repair"}</b><p>{issue.hint}</p>{issue.guidingQuestion && <p><b>Think about:</b> {issue.guidingQuestion}</p>}{issue.partialExample && <p><b>Example with different content:</b> <span lang="de">{issue.partialExample}</span></p>}{attempt.revealed && <div><p lang="de">{issue.original} → <strong>{issue.corrected}</strong></p><p>{issue.explanation}</p></div>}
      {attempt.disputes?.some((item) => item.start === issue.start) ? <p role="status">Concern saved. This correction is excluded from your learning profile. It has not been reviewed by a teacher.</p> : <ChallengeCorrection key={`${attempt.id}:${issue.start}`} start={issue.start} busy={busy} onAction={onAction} />}
    </article>)}
    ...
    <p className="writing-guidance">{attempt.revealed ? "Correction viewed. Later revisions are recorded as correction-assisted." : "Use the hints to repair your response."} Same-task repetition does not prove independent reuse.</p>
  </section>;
}
