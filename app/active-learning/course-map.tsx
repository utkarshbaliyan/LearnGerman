 ...
 return <>
  ...
  <div className="active-modules">{Array.from({length:12},(_,i) => i+1).map(module => {
   ...
   return <details key={`${level}-${module}`} className="active-module" open={next?.row.module === module || undefined}>
    ...
    {level === 'A1' && module === 1 && <div className="starter-module-check"><b>Try a new introduction</b><p>Introduce yourself in a different situation.</p>{modes.map(mode => <Link key={mode} href={`/active-learning/${starterCheckpoint.id}?mode=${mode}`}>{progress[starterCheckpoint.id]?.[mode]?.checked ? '✓ ' : ''}{mode === 'speaking' ? 'Speak' : 'Write'}</Link>)}</div>}
   </details>;
  })}</div>
