...
export function ActiveLessonWorkspace({lesson,initialMode}:{lesson:ActiveLesson;initialMode:ActiveMode}) {
 ...
 return <div className="site-shell"><SiteHeader active="active-learning"/><main className="active-learning active-lesson">
  ...
  {completed && <section className="active-finish"><div><Check/><div><h2>You’ve practised this task.</h2><p>Try the correction once more, or stop here for today.</p></div></div><div className="active-finish-actions"><Link href="/active-learning">Done for today</Link>{next && <Link className="active-primary" href={`/active-learning/${next.id}?mode=${next.mode}`}>Next activity<ArrowRight size={16}/></Link>}</div></section>}
 </main></div>;
}
