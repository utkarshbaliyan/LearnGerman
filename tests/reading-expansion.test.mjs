...
test('A1/A2 expanded stories and the B1 catalog preserve topics and progress', async () => {
 ...
 try {
  const {READING_STORIES,READING_SECTIONS,readingSummary,readingWordCount,getChapterReading} = await vite.ssrLoadModule('/app/lib/reading-path.ts');
  ...
  for(const [level,total] of [['A1',104],['A2',150],['B1',200]]) {
   ...
   for(const story of expansion) {
    ...
    if (range) assert.ok(story.text.split(/\s+/).length>=range[0]);
  ...
  assert.equal(filterReadingStories(summaries,'A1','all','').length,104);
  ...
  progress=mergeStoryProgress(progress,markStory({entries:{}},'a1-001',true,50));
  assert.equal(progress.entries['a1-001'].completed,true,'retired story completion remains in synced account data');
